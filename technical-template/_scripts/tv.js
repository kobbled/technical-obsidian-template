...
async function start(params, settings) {
    ...
    const query = await QuickAdd.quickAddApi.inputPrompt("Enter TV title or IMDB ID: ");
    if (!query) {
    ...
    const isWatched = await QuickAdd.quickAddApi.yesNoPrompt("Watched ?");
	let myRating = " ";
    ...
    QuickAdd.variables = {
        ...
        tag: `${formatList(selectedShow.Genre.split(",").map(item => 
            "TV/Genre/" + 
            item.trim().toLowerCase().replace(/\s+/g, "-").replace(/[()]/g, "")
          ), false)}`,
    }
}
