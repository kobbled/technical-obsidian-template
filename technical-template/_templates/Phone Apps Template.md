---
<%*
  const folderPath = "Personal/Phone/apps";
  let type = await tp.system.prompt("Type of App?")
  const type_tag = type.toLowerCase().replace(/[^a-zA-Z0-9]/g, "-"); 
  
  let os = await tp.user.multiSuggester(tp, ["android", "iOS"], ["\"android\"", "\"iOS\""], false, "Operating System")
...
usage: <%`${usage}`%>
References:
Tags: [note/info, phone/apps, phone/apps/<%`${type_tag}`%>]
Links: 
---
